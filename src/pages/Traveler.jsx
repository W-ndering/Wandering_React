...
    }
    ...
    // 컷 ID 12의 선택지 처리: '산을 내려가자' 클릭 시 내비게이션
    if (current.id === 12 && choiceIndex !== null) {
        navigate("/climbdown");
        return; // 내비게이션 실행 후 함수 종료
    }
    
    // 마지막 컷이 아니라면 다음 컷으로 인덱스 이동
...
  // 마지막 컷에서 우측 끝 도달 시 다음 페이지로 이동 (컷 ID 12에서 이동은 선택지 클릭으로 대체되었지만, 로직은 유지)
  useEffect(() => {
    if (current.id !== 12) return;
    const EDGE = maxX - 5;
